...
namespace WindowsFormsApp1
{
    ...
    FastColoredTextBox CurrentTB
        ...
        {
            //run_regex("(\\d{4}\\d{6}\\d{5})");
            avtomat_american();
        }

        private void button1_Click_1(object sender, EventArgs e)
        {
            run_regex("(\\d{3}-\\d{3}-\\d{3} \\d{2})+");
        }

        private void button3_Click(object sender, EventArgs e)
        {
            avtomat_mac_adress();
            //run_regex("[\\d\\w]{2}-[\\d\\w]{2}-[\\d\\w]{2}-[\\d\\w]{2}-[\\d\\w]{2}-[\\d\\w]{2}");
        }

        private void run_regex(string pattern)
        {
            Match match = Regex.Match(CurrentTB == null ? "" : CurrentTB.Text, pattern);
            if (match.Success)
            {
                MessageBox.Show("Есть вхождение строки!");
                return;
            }
            MessageBox.Show("Вхождение строки не найдено!");
        }


        private void avtomat_mac_adress()
        {
            MacAdressParser macAdressParser = new MacAdressParser();
            if(macAdressParser.parse(CurrentTB.Text))
                MessageBox.Show("Есть вхождение строки!");
            else
                MessageBox.Show("Вхождение строки не найдено!");
        }

        private void avtomat_american()
        {
            AmericanExpressParser macAdressParser = new AmericanExpressParser();
            if (macAdressParser.parse(CurrentTB.Text))
                MessageBox.Show("Есть вхождение строки!");
            else
                MessageBox.Show("Вхождение строки не найдено!");
